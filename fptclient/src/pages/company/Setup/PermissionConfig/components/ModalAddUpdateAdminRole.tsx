...
const ModalAddUpdateAdminRole = ({ role, screen }: { role?: RoleRes; screen: ScreenType }) => {
  ...
  if (role) {
    form.setFieldsValue({ ...role, permissions: undefined, status: role.status === Status.ACTIVE });
  }
  ...
  return (
    <Form form={form} autoComplete="off" labelCol={{ span: 4 }} layout="horizontal" onFinish={handleSubmitForm}>
