...
import { app, navigateToPrivateRoute } from 'routes';
import { openModal } from 'store/slices/modalSlice';
import { RootState } from 'types/store';
import ModalChangePassword from './ModalChangePassword';
import ModalUpdateCompanyProfile from './ModalUpdateCompanyProfile/index';
...
const AppHeader = memo(() => {
    ...
    const items: MenuProps['items'] = [
      {
        ...
        onClick: () =>
          openModal({
            content: app === APP.ADMIN ? <ModalUpdateProfile /> : <ModalUpdateCompanyProfile />,
            options: {
              width: 1000,
  ...
  return (
    <div className="flex h-full items-center justify-between px-8">
      <Link to={navigateToPrivateRoute()} className="flex justify-center">
